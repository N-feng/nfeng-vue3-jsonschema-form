...
// import { isObject, hasOwnProperty, getSchemaType, guessType } from
...
export function resolveSchema(schema: Schema, rootSchema = {}, formData = {}) {
  if (hasOwnProperty(schema, '$ref')) {
    ...
    const resolveSchema = resolveDependencies(schema, rootSchema, formData)
    return retrieveSchema(resolveSchema, rootSchema, formData)
    ...
    // No $ref or dependencies attribute found, returning the original
    return schema
  }
}
