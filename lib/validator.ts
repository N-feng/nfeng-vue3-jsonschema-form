...
function toErrorSchema(errors: TransformedErrorObject[]) {
  ...
  return errors.reduce((errorSchema, error) => {
    ...
    // If eht property is at the root (.level1) then toPath creates
    ...
    if (Array.isArray(parent.__errors)) {
      ...
      // to avoid name collision with possible sub schema field named
...
export function mergeObjects(obj1: any, obj2: any, concatArrays = false) {
  ...
  const acc = Object.assign({}, obj1) // Prevent mutations of source object.
